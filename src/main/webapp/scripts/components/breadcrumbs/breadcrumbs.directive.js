        ...
        function link(scope) {
            ...
            function updateBreadcrumbsArray() {
                ...
                while(currentState && currentState.name !== '') {
                    //console.log("cs="+ currentState.name +"  display=" + currentState.data.displayName + "  parent=" + currentState.parent);
                    workingState = getWorkingState(currentState);

                    if (workingState) {
                        workingState = adapt(workingState);
                        displayName = getDisplayName(workingState);

                        //console.log("dealing with : " + workingState.name+ "["+displayName+"]")
                        if (displayName !== false && !stateAlreadyInBreadcrumbs(workingState, breadcrumbs)) {
                            saveStateInBreadcrumbs(breadcrumbs, displayName, workingState);
                        }
                    }
                    currentState = $state.get(currentState.parent);
                }
                pushEmpty(breadcrumbs);
                breadcrumbs.reverse();
            ...
            function getWorkingState(currentState) {
                ...
                //console.log("picked -> " + workingState.name)
                return workingState;
            }
            ...
            function saveStateInBreadcrumbs(breadcrumbs, displayName, workingState) {
                    //console.log("----------> pushed");
                    breadcrumbs.push({
            ...
            function pushEmpty(breadcrumbs) {
                //saveStateInBreadcrumbs(breadcrumbs, "", {name:""});
            }

            function adapt(workingState) {
                if (workingState.name === "site") {
                    workingState.name = "home";
                }
                return workingState;
            }
        }
